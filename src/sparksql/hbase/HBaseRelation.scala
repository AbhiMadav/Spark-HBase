...
case class DataFrameSchemaField(fieldName: String, fieldType: String) extends SchemaField with Serializable
...
object Resolver extends Serializable {
  ...
  def resolveRowKey(result: Result, resultType: String): Any = {
    val rowkey = resultType match {
      ...
      case "long" =>
        result.getRow.map(_.toChar).mkString.toLong //Test ByteBuffer.wrap(result.getRow).getLong
    }
    rowkey
  }
    ...
    val column = result.containsColumn(columnFamily.getBytes, columnName.getBytes) match {
      case true => {
        resultType match {
          ...
          case "binary" =>
            (result.getValue(columnFamily.getBytes, columnName.getBytes)) //TODO: protobuf the result?
          case "boolean" =>
            Bytes.toBoolean(result.getValue(columnFamily.getBytes, columnName.getBytes))
        }
      }
      case _ => {
        //XXX Should return null if not found anything.
        resultType match {
          case "string" =>
            ""
          case "int" =>
...
case class HBaseRelation(@transient val hbaseProps: Map[String, String])(@transient val sqlContext: SQLContext) extends BaseRelation with Serializable with TableScan {
  ...
  val zookeeperQuorumHost = hbaseProps.getOrElse("zookeeperQuorum", "localhost")
  ...
  def feedTypes(mapping: Map[HBaseSchemaField, DataFrameSchemaField]): Array[HBaseSchemaField] = {
  ...
  def tableSchemaFieldMapping(externalHBaseTable: Array[HBaseSchemaField], registerTable: Array[DataFrameSchemaField]): Map[HBaseSchemaField, DataFrameSchemaField] = {
   ...
   *   convert into [DataFrameSchemaField{rowkey, string}, DataFrameSchemaField{value, string}...]
   */
  def extractRegisterSchema(registerTableSchema: String): Array[DataFrameSchemaField] = {
    ...
    fieldsArray.map { fildString =>
      ...
      DataFrameSchemaField(splitedField(0), splitedField(1))
    }
  }
  ...
  lazy val buildScan = {
    ...
    hbaseConf.set(HConstants.ZOOKEEPER_QUORUM, zookeeperQuorumHost)
